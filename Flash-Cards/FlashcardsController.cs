...
namespace flashcards
{
    public class FlashcardsController
    {
            ...
            while (CreateFlashcard)
            {
                ...
                using (conn)
                {
                    ...
                    tableCmd.CommandText =
                        $@"INSERT INTO flashcard (question, answer, stackId) VALUES ('{flashcard.Question}', '{stackId}')";
        ...
        {
            int flashcardIdOnView = UserCommands.GetIntergerInput("\nWhich flashcard would you like to update?");
        ...
        internal static void UpdateFlashcard(List<FlashcardsWithStack> list)
        {
            ...
            string questionOption = UserCommands.GetBinaryInput("Would you like to update the question? (Y/N)");

            if (questionOption == "Y");
                newQuestion = UserCommands.GetStringInput("Please type new question:");

            string answerOption = UserCommands.GetBinaryInput("Would you like to update the answer? (Y/N)");
            if (answerOption == "Y");
                newAnswer = UserCommands.GetStringInput("Please type a new answer:");
